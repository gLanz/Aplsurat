...
class Customer
{
    ...
    {
        $rset = $this->mysql->select('*',"co_menu");
        return $rset;
    }
    ...
    {
        $rset = $this->mysql->select('*',"co_board","me_id='$id'");
        return $rset;
    }
    ...
    public function addCustomer($post)
    {
        ...
        if( $this->mysql->insert("co_board",$fields) ) // This is pretty basic. Inserts the new news record.
        {
            return true;
        }
        else
    ...
    public function editCustomer($post)
    {
        ...
        $newsId = $post['idboard']; // retreive the newsId we need to update
        if( $this->mysql->update("co_board",$fields,"idboard='$newsId'") )
        {
            return true;
        }
        else
